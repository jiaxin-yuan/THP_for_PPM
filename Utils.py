...
def log_likelihood(model, data, time, types):
    ...
    all_lambda = softplus(all_hid, model.beta) #  shape: [batch_size, seq_len, num_types]， the intensity for each type of event at each time step
    type_lambda = torch.sum(all_lambda * type_mask, dim=2) # shape: [batch_size, seq_len]， the intensity for the actual event type at each time step

    # event log-likelihood
    event_ll = compute_event(type_lambda, non_pad_mask) # shape: [batch_size, seq_len]， the log-likelihood of the actual events at each time step
    event_ll = torch.sum(event_ll, dim=-1) # shape: [batch_size]， the log-likelihood of the actual events for each sequence in the batch

    # non-event log-likelihood, either numerical integration or MC integration
    # non_event_ll = compute_integral_biased(type_lambda, time, non_pad_mask)
    non_event_ll = compute_integral_unbiased(model, data, time, non_pad_mask, type_mask) # shape: [batch_size, seq_len-1]， the log-likelihood of non-events between each pair of consecutive events
    non_event_ll = torch.sum(non_event_ll, dim=-1) # shape: [batch_size]， the log-likelihood of non-events for each sequence in the batch
